...
class Siegenia extends utils.Adapter {
      ...
      dev.getDeviceInfo((err, status, data) => {
        ...
        if (status !== "ok") {
          ...
          callback &&
            callback({
              ip: ip,
              name: `${data.devicename}(${data.devicefloor} ${
                data.devicelocation
              })`,
            });
          callback = null;
  ...
  initObjects(device, channel, data, command) {
    ...
    this.log.debug(
      `Objects for ${device.ip}/${command}/${channel}: ${JSON.stringify(objs)}`,
    );
    ...
    for (const key in objs) {
      ...
      if (objs[key].write) {
        onChange = (value) => {
          ...
          this.log.debug(
            `onStateChange Device ${device.ip}: ${JSON.stringify(value)}`,
          );
          this.devices[device.ip].comm.setDeviceParams(
            value,
            (err, status, data) => {
              if (err) {
                this.log.error(err);
              ...
              if (status !== "ok") {
                this.log.error(
                  `set Device Parameter Error for device ${device.ip}: ${
                    status
                  }`,
                );
              }
    ...
    device.comm.getDeviceInfo((err, status, data) => {
      ...
      if (status !== "ok") {
        return (
          ...
          callback(
            new Error(
              `Get Device Info Error for device ${device.ip}: ${status}`,
            ),
          )
        );
      }
      ...
      if (!data.type || !Mapper.DeviceTypeMap[data.type]) {
        return (
          ...
          callback(
            new Error(
              `Unknown device type for device ${device.ip}: ${data.type}`,
            ),
          )
        );
      }
        ...
        (err, status, data) => {
          ...
          if (status !== "ok") {
            return (
              callback &&
              callback(
                new Error(`Login Error for device ${device.ip}: ${status}`),
              )
            );
          }
          ...
          device.comm.getDeviceState((err, status, data) => {
            ...
            if (status !== "ok") {
              return (
                ...
                callback(
                  new Error(
                    `Get Device State Error for device ${device.ip}: ${status}`,
                  ),
                )
              );
            }
            ...
            device.comm.getDeviceParams((err, status, data) => {
              ...
              if (status !== "ok") {
                return (
                  ...
                  callback(
                    new Error(
                      `Get Device Parameters Error for device ${device.ip}: ${
                        status
                      }`,
                    ),
                  )
              ...
              device.comm.getDeviceDetails((err, status, data) => {
                ...
                if (status !== "ok") {
                  return (
                    ...
                    callback(
                      new Error(
                        `Get Device Details Error for device ${device.ip}: ${
                          status
                        }`,
                      ),
                    )
                ...
                this.objectHelper.setOrUpdateObject(
                  ...
                  false,
                  (value) => {
                    this.devices[device.ip].comm.rebootDevice(
                      (err, status, data) => {
                        if (err) {
                          this.log.error(err);
                        }
                        if (status !== "ok") {
                          this.log.error(
                            `Reboot Error for device ${device.ip}: ${status}`,
                          );
                        }
                        this.setState(
  ...
  initDevice(device, callback) {
    ...
    device.comm.on("closed", (code, reason) => {
      this.log.info(
        `Connection to Device ${device.ip}: CLOSED ${code} / ${reason}`,
      );
    ...
    device.comm.on("data", (status, data, command) => {
      this.log.debug(
        `DATA for ${device.ip}:${command} / ${status} / ${JSON.stringify(
          data,
        )}`,
      );
      if (data) {
        ...
        const states = Mapper.mapToStates(
          command,
          this.devices[device.ip].type,
          data,
        );
        this.log.debug(
          `Set States for ${device.ip}: ${JSON.stringify(states)}`,
        );
        for (const key in states) {
          ...
          this.setState(
            `${this.devices[device.ip].id}.params.${key}`,
            states[key],
            true,
          );
        }
