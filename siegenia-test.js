...
dev.on('connected', () => {
    ...
    dev.resetDevice((err, status, data) => logger(err, status, data, 'resetDevice'));
    dev.rebootDevice((err, status, data) => logger(err, status, data, 'rebootDevice'));
    dev.renewCert((err, status, data) => logger(err, status, data, 'renewCert'));
    dev.getDeviceInfo((err, status, data) => logger(err, status, data, 'getDevice'));
    dev.getDeviceParams((err, status, data) => logger(err, status, data, 'getDeviceParams'));
    dev.getDeviceDetails((err, status, data) => logger(err, status, data, 'getDeviceDetails'));

    dev.setDeviceParams({'devicestate': {'deviceactive': true}}, (err, status, data) => logger(err, status, data, 'setDeviceParams'));
});
...
dev.on('data', (status, data, command) => {
    ...
    if (data) {
        console.log('      ' + JSON.stringify(Mapper.mapToStates(command, 1, data)));
    }
});
...
function logger(err, status, data, command) {
    ...
    if (data) {
        console.log('      ' + JSON.stringify(Mapper.mapToStates(command, 5, data)));

        const objs = Mapper.mapToObjects(command, 5, data);
        console.log('      ' + JSON.stringify(objs));
        for (const obj in objs) {
            if (!objs.hasOwnProperty(obj)) continue;
            if (objs[obj].write === false) continue;
            console.log('Remap Field ' + obj + ' : ' + JSON.stringify(Mapper.mapValueForWrite(obj, objs[obj].value, objs[obj].native)));
        }
    }
}