...
#### Key Features
- **Automatic Device Discovery**: The adapter automatically detects Siegenia devices in the same network using mDNS (via `mdns-js` library)
- **WebSocket Communication**: Uses WebSocket protocol (`ws` library) for real-time communication with devices
- **Authentication**: Requires user credentials for device access
- **Data Polling**: Continuously polls device data and updates ioBroker states
- **State Management**: All available device data fields are exposed as ioBroker states with read/write capabilities where applicable

#### Technical Details
- **Connection Type**: Local network communication
- **Protocol**: WebSocket-based communication with Siegenia devices
- **Device Detection**: mDNS/Bonjour service discovery
- **Minimum Node.js**: 18.x (as per package.json engines)
- **Dependencies**:
  - `@iobroker/adapter-core` for adapter base functionality
  - `ws` for WebSocket communication
  - `mdns-js` for device discovery
  - `@apollon/iobroker-tools` for helper functions

#### Configuration
- Devices can be discovered automatically or configured manually with IP address, username, and password
- Admin interface allows correcting user/password after automatic discovery
- Each device appears under a device-specific namespace with its parameters exposed as states

#### Development Considerations
- Handle device connection errors gracefully (authentication failures, network issues)
- Implement proper cleanup in the `unload()` method to disconnect from devices
- Use appropriate logging levels for device discovery, connection status, and data updates
- Timers and complex data structures are displayed but can only be changed via the Siegenia App
