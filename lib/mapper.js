...
const deviceSpecialObjects = {
    '3': { // DRIVE axxent Family, Seite 20
        'openclose': {read: false, write: true, type: 'number', role: 'level', states: {1: 'OPEN', 2: 'CLOSE', 3: 'OPEN_TO_TURN'}, native: {handleAsString: true, states: {1: 'OPEN', 2: 'CLOSE', 3: 'OPEN_TO_TURN'}}},
        'stop': {read: false, write: true, type: 'boolean', role: 'button.stop'}
    },
    '5': { // AEROVITAL ambience
        // Timer-delete und so ??? TODO
        'timer.fanpower': {read: false, write: true, type: 'number', role: 'level', unit: '%', min: 0, max: 100},
        'timer.fanmode': {read: false, write: true, type: 'number', role: 'level', states: {1: 'IN', 2: 'OUT', 3: 'IN_OUT', 4: 'IN_OUT_WRG', 5: 'AUTO'}, native: {handleAsString: true, states: {1: 'IN', 2: 'OUT', 3: 'IN_OUT', 4: 'IN_OUT_WRG', 5: 'AUTO'}}}
    },
    '6': { // MHS Family
        'sash-0.openclose': {read: false, write: true, type: 'number', role: 'level', states: {1: 'OPEN', 2: 'GAP_VENT', 3: 'STOP_OVER', 4: 'CLOSE', 5: 'CLOSE_WO_LOCK'}, native: {handleAsString: true, realId: 'openclose.0', states: {1: 'OPEN', 2: 'GAP_VENT', 3: 'STOP_OVER', 4: 'CLOSE', 5: 'CLOSE_WO_LOCK'}}},
        'sash-1.openclose': {read: false, write: true, type: 'number', role: 'level', states: {1: 'OPEN', 2: 'GAP_VENT', 3: 'STOP_OVER', 4: 'CLOSE', 5: 'CLOSE_WO_LOCK'}, native: {handleAsString: true, realId: 'openclose.1', states: {1: 'OPEN', 2: 'GAP_VENT', 3: 'STOP_OVER', 4: 'CLOSE', 5: 'CLOSE_WO_LOCK'}}},
        'sash-0.stop': {read: false, write: true, type: 'boolean', role: 'button.stop', native: {realId: 'stop.0'}},
        'sash-1.stop': {read: false, write: true, type: 'boolean', role: 'button.stop', native: {realId: 'stop.1'}}
    },
    '8': { // AEROTUBE
        // Timer-delete und so ??? TODO
        'timer.fanpower': {read: false, write: true, type: 'number', role: 'level', unit: '%', min: 0, max: 100},
        'timer.fanmode': {read: false, write: true, type: 'number', role: 'level', states: {1: 'IN', 2: 'OUT', 3: 'IN_OUT', 4: 'IN_OUT_WRG', 5: 'AUTO'}, native: {handleAsString: true, states: {1: 'IN', 2: 'OUT', 3: 'IN_OUT', 4: 'IN_OUT_WRG', 5: 'AUTO'}}},
        'timer.duration': {type: 'value', role: 'state', native: {handleAsTimehhmm: true, maxHour: 18}}
    }
};
...
const fieldMap = {
    ...
    'getDeviceState': {
        'deviceactive': {id: 'active', type: 'boolean', read: true, write: true, def: false, native: {realId: 'devicestate.deviceactive'}}
    },
    'getDeviceParams': {
        'all': {read: true, write: true},
        // Definitions that work for all relevant devices
        'devicestate.deviceactive': {id: 'active', type: 'boolean', read: true, write: true, def: false, native: {realId: 'devicestate.deviceactive'}},
        'warnings': {type: 'array', write: false},
        'airbase.temperature.indoor': {type: 'number', role: 'value.temperature', unit: '◦C', write: false},
        'airbase.temperature.outdoor': {type: 'number', role: 'value.temperature', unit: '◦C', write: false},
        'airbase.humidity.indoor': {type: 'number', role: 'value.humidity', unit: '%', write: false},
        'airbase.humidity.outdoor': {type: 'number', role: 'value.humidity', unit: '%', write: false},
        ...
        '1': { // AEROPAC
            ...
            'timer.duration': {type: 'value', native: {handleAsTimehhmm: true, maxHour: 18}},
            'timer.remainingtime': {type: 'value', write: false},
            'timer.poweron_time': {type: 'value', native: {handleAsTimehhmm: true}},
            'clock': {ignore: true}
        },
        '2': { // AEROMAT VT
            'fanpower': {type: 'number', role: 'level.valve', unit: '%', min: 0, max: 100},
            'maxfanpower': {type: 'number', role: 'level', unit: 'm3/h', min: 0},
            'airquality.co2content': {type: 'number', role: 'value.co2', unit: 'PPM', write: false},
            'airquality.voc': {type: 'number', role: 'value', write: false},
        },
        '3': { // DRIVE axxent DK/MH
        ...
        '5': { // AEROVITAL ambience
            'fanpower': {type: 'number', role: 'level.valve', unit: '%', min: 0, max: 100},
            'fanmode': {type: 'number', states: {1: 'IN', 2: 'OUT', 3: 'IN_OUT', 4: 'IN_OUT_WRG'}, native: {handleAsString: true}},
            'maxfanpower': {type: 'number', role: 'value', unit: 'm3/h', write: false},
            'automode_maxairflow': {type: 'number', role: 'level.valve', unit: '%', min: 0, max: 100},
            'automode_co2sensity': {type: 'number', role: 'level', unit: '%', min: 0, max: 100},
            'lighting.front': {type: 'string', role: 'level.color.rgb', native: {handleAsColor: true}},
            'lighting.back': {type: 'string', role: 'level.color.rgb', native: {handleAsColor: true}},
        ...
        '6': { // MHS Family
            ...
            'stopover': {unit: 'dm'},
            'timer.duration': {type: 'value', native: {handleAsTimehhmm: true, maxHour: 4}},
            'timer.remainingtime': {type: 'value', write: false}
        },
        '8': { // AEROTUBE
            'fanpower': {type: 'number', role: 'level.valve', unit: '%', min: 0, max: 100},
            'fanmode': {type: 'number', states: {1: 'IN', 2: 'OUT', 3: 'IN_OUT', 4: 'AUTO'}, native: {handleAsString: true}},
            'fanmirror': {type: 'number', states: {0: 'Slave mode', 1: 'Slave must mirror', 2: 'Slave must copy'}},
            'slave_fanpower': {type: 'number', role: 'level.valve', unit: '%', min: 0, max: 100},
            'slave_fandirection': {type: 'number', states: {1: 'Slave supply air', 2: 'Slave exhaust air'}},
            'maxfanpower': {type: 'number', role: 'value', unit: 'm3/h', write: false},
            'automode_maxairflow': {type: 'number', role: 'level.valve', unit: '%', min: 0, max: 100},
            'automode_co2sensity': {type: 'number', role: 'level', unit: '%', min: 0, max: 100},
            'bathcontrolfanpower': {type: 'number', role: 'level.valve', unit: '%', min: 0, max: 100},
            'bathcontrolmodeactive': {type: 'number', states: {1: 'IN', 2: 'OUT', 3: 'IGNORE'}, native: {handleAsString: true}},
            'bathcontrolmodepassive': {type: 'number', states: {1: 'IN', 2: 'OUT', 3: 'IN_OUT', 4: 'AUTO'}, native: {handleAsString: true}},
            'clock': {ignore: true},
            'ecomode_maxairflow': {type: 'number', role: 'level.valve', unit: '%', min: 0, max: 100},
        ...
        10: { // Universal Module
            'fanmode': {type: 'number', states: {1: 'OPEN', 2: 'CLOSED'}, write: false}
        }
    }
};
...
function flattenFieldNames(data, prefix, fields) {
    ...
    if (data['hour'] !== undefined && data['minute'] !== undefined && data['year'] !== undefined && data['month'] !== undefined && data['day'] !== undefined) {
...
function mapValueForRead(value, def) {
    ...
    if (def.native.handleAsColor) {
        ...
        for (const key in def.states) {
            if (!def.states.hasOwnProperty(key)) continue;
...
function mapValueForWrite(name, value, native) {
    if (native && native.handleAsColor) {
        ...
        if (valArr.length < 2 || valArr.length > 3) return;
        value = {
