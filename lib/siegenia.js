    ...
    this.heartbeatTimeout = setTimeout(() => {
      ...
      this.sendRequest("keepAlive", { extend_session: true }, (err, status) => {
        ...
        if (status !== "ok") {
          return this.emit(
            "error",
            new Error(`Response from hartbeat "${status}" is not ok`),
          );
        }
  ...
  connect(callback) {
    if (this.websocket) {
      return (
        ...
        callback(
          new Error(
            "initializeCall for Websocket, but Websocket already set, so ignore call",
          ),
        )
      );
    }
    ...
    this.websocket = new ws(
      `${this.wsProtocol}://${this.ip}:${this.port}/WebSocket`,
      {
    ...
    this.websocket.on("unexpected-response", (request, response) => {
      this.logger(
        `${this.ip}: Websocket Unexpected Response: ${response.statusCode} / ${
    ...
    this.websocket.on("message", (data) => {
      ...
      if (dataObj && dataObj.id && this.awaitingResponses[dataObj.id]) {
        ...
        this.awaitingResponses[dataObj.id].callback(
          null,
          dataObj.status,
          dataObj.data,
        );
